...
fn parse_forrest(forrest_in: &Vec<String>, size: usize) -> (u32, u32) {
    
    let mut forrest = vec![vec![0; size]; size];
    let mut forrest_t = vec![vec![0; size]; size];
    for i in 0..size {
...
pub fn run() {
    ...
    let (num_visible_trees, max_scenic_score) = parse_forrest(&lines, 99);
