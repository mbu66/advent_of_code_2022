...
fn parse_topology_down(topology: &Vec<Vec<i32>>, start: (usize, usize),) -> i32 {
    let ( mut distances, mut active_coords, mut end_reached, mut end) = (vec![vec![0; topology[0].len()]; topology.len()], vec![start], false, (0, 0));
...
fn parse_topology_up(topology: &Vec<Vec<i32>>, start: (usize, usize), end: (usize, usize)) -> i32 {
    let (mut distances, mut active_coords, mut end_reached) = (vec![vec![0; topology[0].len()]; topology.len()], vec![start], false);
    while end_reached == false {
        let local_coords = active_coords.clone();
        active_coords.clear();
        for coord in local_coords {
